...
void selectionSort(CensusData CenDataArr[], int size)
{
	...
	for (int startScan = 0; startScan < (size - 1); startScan++)
	{
		...
		for(int index = startScan + 1; index < size; index++)
		{
			...
			{
				minIndex = index;   // update where we found the lowest value
			}
		} // end inner "for" loop

		if (minIndex != startScan){     // Check to see if the lowest value was not at the beginning
            minValue[0] = CenDataArr[minIndex];
            CenDataArr[minIndex] = CenDataArr[startScan];
            CenDataArr[startScan] = minValue[0];
            //	num1 = std::stoi(CenDataArr[minIndex].adultPop);
            swapsMade++;
		}
    } // end outer "for" loop
