...
void startarray(CensusData CenDataArr[])
{
    ...
    ifstream MyData("CensusData.csv");
...
{
    CensusData temp; //temporary variable is struct object to hold values
...
{
    int swapsMade = 0;
    for (int startScan = 0; startScan < (size - 1); startScan++) /* startScan is where we will be placing the struct
                                                                     with the next smallest adult population */
    {
        int minIndex = startScan; /* minIndex is the index in CenDataArr tith the next smallest
                                    adult population*/
        int num1 = std::stoi(CenDataArr[minIndex].adultPop); // variable to hold
        for (int index = startScan + 1; index < size; index++)
        {
            //Convert the string variables into usable integer types for comparison (similar to bubbleSort conversion)
            int num2 = std::stoi(CenDataArr[index].adultPop);
        ...
        CensusData minValue = CenDataArr[minIndex];
        CenDataArr[minIndex] = CenDataArr[startScan];
        CenDataArr[startScan] = minValue;
        swapsMade++;
    }
